'''
python main.py --strategy SL_Bayesian --data-dir DATA/PART1 --output-dir output/ShortLookback
'''

'''
{
  "final_value": 1026579.9469699998,
  "bankrupt": false,
  "bankrupt_date": null,
  "open_pnl_pd_ratio": 1.1421699243388523,
  "true_pd_ratio": 1.1168843475451442,
  "activity_pct": 96.7,
...
class BayesianRegimeDetector(object):
    ...
    def __init__(self, lookback=40, alpha=2.0, beta=2.0):
...
class BayesianTradingStrategy(bt.Strategy):
    params = dict(
        ...
        atr_len=10,              # ATR lookback period for volatility
        atr_k=2.1,               # Stop loss distance: 2.1 * ATR below entry
        meanrev_window=5,        # Look back 5 bars for recent performance
        top_pct=0.20,            # Target bottom 20% of stocks 
        volume_z=0.8,            # Volume must be 0.8 std dev above average
        short_return=-0.015,      # Stock must be down 1.5%+ in last 5 bars
        ...
        breakout_lookback=6,     # Compare current close to highest of last 6 bars
        ma_len=15,               # 15-bar moving average for trend confirmation
        ...
        bayesian_lookback = 40,          # Rolling Lookback for Bayesian Regime Detector
        ...
        min_confidence_mr=0.44,          # Mean reversion signal must have >44% posterior accuracy
        min_confidence_trend=0.44,       # Trend signal must have >44% posterior accuracy
        sideways_threshold=0.35,         # Enter mean reversion if sideways probability >35%
        trending_threshold=0.55,         # Enter trend following if (uptrend + downtrend) >55%
