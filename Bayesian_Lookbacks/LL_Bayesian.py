'''
python main.py --strategy LL_Bayesian --data-dir DATA/PART1 --output-dir output/LongLookback
'''

'''

{
  "final_value": 1015073.7533160002,
  "bankrupt": false,
  "bankrupt_date": null,
  "open_pnl_pd_ratio": 0.6268014471474942,
  "true_pd_ratio": 0.6076771949595149,
  "activity_pct": 90.6,
...
class BayesianRegimeDetector(object):
    ...
    def __init__(self, lookback=120, alpha=2.0, beta=2.0):
...
class BayesianTradingStrategy(bt.Strategy):
    params = dict(
        ...
        atr_len=30,              # ATR lookback period for volatility
        atr_k=3.0,               # Stop loss distance: 3.0 * ATR below entry
        meanrev_window=30,       # Look back 30 bars for recent performance
        top_pct=0.10,            # Target bottom 10% of stocks 
        volume_z=0.9,            # Volume must be 0.9 std dev above average
        short_return=-0.030,     # Stock must be down 3%+ in last 5 bars
        long_return=0.0,         # Stock must be up 0%+ in last 30 bars 
        bounce_pct=0.025,        # Stock must have bounced 2.5% from 20-bar low
        
        # Trend Following Params
        up_window=20,             # Look back 20 bars
        up_days_min=13,           # Need at least 13 of last 20 bars higher
        breakout_lookback=30,     # Compare current close to highest of last 30 bars
        ma_len=75,                # 75-bar moving average for trend confirmation
        vol_z_min=0.6,            # Volume must be 0.6 std dev above average
        ...
        # Exit Params
        cooldown=10,                 # Wait 10 bars before trading same stock again
        two_bar_exit=True,           # Require 2 bars of breach before exiting 
        risk_free_bars=14,           # Close all positions 14 bars before backtest end
        end_taper_bars=30,           # Taper exposure in last 30 bars 
        severe_breach_mult=0.60,     # Exit if price drops 0.60*ATR below trailing stop
        
        # Bayesian Learning Params
        bayesian_lookback = 120,         # Rolling Lookback for Bayesian Regime Detector
        ...
        min_confidence_mr=0.38,          # Mean reversion signal must have >38% posterior accuracy
        min_confidence_trend=0.38,       # Trend signal must have >38% posterior accuracy
        sideways_threshold=0.25,         # Enter mean reversion if sideways probability >25%
        trending_threshold=0.40,         # Enter trend following if (uptrend + downtrend) >40%
