'''
python main.py --strategy ML_Bayesian --data-dir DATA/PART1 --output-dir output/MediumLookback
'''

'''
{
  "final_value": 1019226.964156,
  "bankrupt": false,
  "bankrupt_date": null,
  "open_pnl_pd_ratio": 0.803296857939226,
  "true_pd_ratio": 0.7803617414324953,
  "activity_pct": 93.0,
...
class BayesianTradingStrategy(bt.Strategy):
    params = dict(
        ...
        atr_len=21,              # ATR lookback period for volatility
        atr_k=2.7,               # Stop loss distance: 2.7 * ATR below entry
        meanrev_window=7,        # Look back 7 bars for recent performance
        top_pct=0.25,            # Target bottom 25% of stocks 
        volume_z=0.7,            # Volume must be 0.7 std dev above average
        short_return=-0.018,     # Stock must be down 1.8%+ in last 5 bars
        long_return=0.0,         # Stock must be up 0%+ in last 30 bars 
        bounce_pct=0.018,        # Stock must have bounced 1.8% from 20-bar low
        
        # Trend Following Params
        up_window=7,             # Look back 7 bars
        up_days_min=4,           # Need at least 4 of last 7 bars higher
        breakout_lookback=14,    # Compare current close to highest of last 14 bars
        ma_len=30,               # 30-bar moving average for trend confirmation
        ...
        # Exit Params
        cooldown=5,                 # Wait 5 bars before trading same stock again
        two_bar_exit=True,          # Require 2 bars of breach before exiting 
        risk_free_bars=10,          # Close all positions 10 bars before backtest end
        end_taper_bars=21,          # Taper exposure in last 21 bars 
        severe_breach_mult=0.60,    # Exit if price drops 0.60*ATR below trailing stop
        
        # Bayesian Learning Params
        bayesian_lookback = 90,          # Rolling Lookback for Bayesian Regime Detector
        ...
        min_confidence_mr=0.40,          # Mean reversion signal must have >40% posterior accuracy
        min_confidence_trend=0.40,       # Trend signal must have >40% posterior accuracy
        sideways_threshold=0.27,         # Enter mean reversion if sideways probability >27%
        trending_threshold=0.45,         # Enter trend following if (uptrend + downtrend) >45%
