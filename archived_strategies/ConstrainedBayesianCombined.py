...
'''
{
  "final_value": 1014376.4765999999,
  "bankrupt": false,
  "bankrupt_date": null,
  "open_pnl_pd_ratio": 0.6070414378795549,
  "true_pd_ratio": 0.5818780366180797,
  "activity_pct": 95.3,
...
class BayesianTradingStrategy(bt.Strategy):
    ...
    def next(self):
        ...
        for d in self.datas:
            ...
            # MEAN REVERSION: Only trade in sideways regime
            if regime['sideways'] >= self.p.sideways_threshold and d in laggards:
                mr_valid, mr_sigs = self.eval_mr_signals(d)
                if mr_valid:
                    self.enter_trade(d, size_sign=1, entry_type='mean_reversion')
                    continue

            # TREND FOLLOWING: Only trade in trending regime
