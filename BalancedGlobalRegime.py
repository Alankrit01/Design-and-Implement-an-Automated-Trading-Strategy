'''
SuperShortFixed with Global Market Regime detection for risk on/off and Pair Trading 
Widened Laggard Pool and Changed MR stoploss to be much tighter than before - Increased Mean Reversion Trades
...
'''
{
  "final_value": 1111376.8193716798,
  "bankrupt": false,
  "bankrupt_date": null,
  "open_pnl_pd_ratio": 1.742915581730983,
  "true_pd_ratio": 1.6471737165023252,
  "activity_pct": 91.8,
...
--- Portfolio Total ---
  Total positions : 11
  Total wins      : 6
  Overall PnL     : $116,102.01
  Overall win rate: 54.5%

Symbol     Type                     EntryDate    ExitDate              PnL  W/L
--------------------------------------------------------------------------------------------------------------
series_2   mean_reversion_short     2070-02-21   2072-08-27     -5,135.00  LOSS
series_6   trend_following_long     2070-02-21   2072-08-27       -570.63  LOSS
series_3   mean_reversion_long      2070-02-22   2072-08-27       -298.93  LOSS
series_4   trend_following_long     2070-02-22   2072-08-27      6,149.66  WIN
series_1   mean_reversion_long      2070-02-23   2072-08-27    -22,339.27  LOSS
series_8   mean_reversion_long      2070-02-23   2072-08-27      4,106.36  WIN
series_9   mean_reversion_long      2070-02-23   2072-08-27     25,332.00  WIN
series_5   mean_reversion_long      2070-02-24   2072-08-27    106,241.17  WIN
series_10  trend_following_short    2070-03-07   2070-05-30        -12.75  LOSS
series_7   trend_following_long     2070-03-10   2072-08-27         88.16  WIN
series_10  trend_following_long     2070-05-30   2072-08-27      2,541.25  WIN
'''
...
CONFIGS = {
    'LB40': dict(
        ...
        # CHANGE 2: mr_top_pct controls the MR candidate pool independently of top_pct (which gates TF laggard detection).  
        # 0.30 = bottom 3 of 10 assets eligible for MR long on any bar, vs the old effective 1 asset from top_pct=0.10/0.20.
        mr_top_pct=0.30,
        ...
        day_budget_frac=0.30, day_symbol_cap_notional=0.12,
        ...
        # CHANGE 3: MR-specific exit parameters
        mr_max_hold_bars=15,      # hard time-stop: exit after N bars regardless
        mr_profit_target=0.04,    # close at +4% unrealised — take the bounce
        mr_atr_k=0.8,             # tight trailing stop (vs 2.1 for TF)
    ),
    'LB60': dict(
...
class SignalEvaluator:
    ...
    def get_signal(self, d, weight, perf, laggards,
                   ...
                   mr_candidates:   set                    = None):
        """
        Returns (size_sign, entry_type) or (0, '') if no signal.
        Updates regime detector but does NOT modify any position state.

        risk_classifier and global_monitor, when provided, gate and adjust the signal based on the cross-series market-wide regime.

        CHANGE 1 — MR long regime gate:
          allow_mr_long is now driven by the *local* asset's own Bayesian sideways probability, not the global RISK_OFF flag.
          Global RISK_OFF correctly blocks TF longs (trend breakouts in a bear market) but should NOT block MR longs
          (mean-reversion bounces on oscillating assets).  An asset with high local sideways probability is a legitimate 
          MR long candidate regardless of the macro regime — catching falling knives is prevented by check_mr_long's own
          conditions (vol spike, bounce off low, close > open).

        CHANGE 2 — MR candidate pool:
          mr_candidates replaces the laggards set for MR long entry eligibility. mr_candidates is built from mr_top_pct=0.30 
          (bottom 3 of 10 assets), giving 3x more opportunities per bar than the old top_pct=0.10 pool.
        """
        cfg = self.cfg
        ...
        if dispersion > 0.03:
            mr_short_ret_thresh = 0.03

        # CHANGE 1: allow_mr_long is gated on the LOCAL asset's sideways probability, not the global risk_off flag.  
        # The asset must look oscillating (sideways probability at or above the config threshold) for MR long to be permitted.
        # This allows MR longs to fire in bear markets on genuinely choppy assets while still being naturally suppressed 
        # on assets in clean downtrends (where sideways probability will be low and downtrend probability high).
        local_sideways = r['sideways']
        allow_mr_long = local_sideways >= cfg['sideways_threshold']

        # TF longs remain blocked by global RISK_OFF (unchanged behaviour)
        allow_tf_long = (not risk_off) if risk_classifier is not None else True

        allow_mr_short = True
        allow_tf_short = True
        # When market is broadly strong, raise the bar for shorts
        if breadth > 0.70:
            allow_mr_short = False
            allow_tf_short = breadth < 0.85

        # CHANGE 2: use mr_candidates (wider pool) for MR long eligibility check.
        # Fall back to laggards if mr_candidates not supplied (backwards compat).
        mr_pool = mr_candidates if mr_candidates is not None else laggards

        # Mean reversion
        if r['sideways'] >= cfg['sideways_threshold']:
            # CHANGE 1+2: MR long uses local sideways gate + wider mr_pool
            if allow_mr_long and d in mr_pool and self.check_mr_long(d):
...
class SuperBayesian(bt.Strategy):
    ...
    def check_exit(self, sym, d, bar):
        ...
        CHANGE 3 — MR positions follow a dedicated short-horizon exit path:
          MR exit triggers (any one sufficient):
            a. mr_max_hold_bars hard time-stop (15–20 bars depending on config)
            b. Profit target: unrealised >= mr_profit_target (e.g. +4%)
            c. Mean-reversion completion: close crosses back through the N-bar SMA
               (the spread has reverted, thesis fulfilled)
            d. Tight trailing stop breach (mr_atr_k = 0.8–0.9 vs 2.1–3.0 for TF)
               with standard two-bar confirmation

        TF positions keep the original exit logic unchanged:
          1. max_hold_bars hard cap
          2. Severe breach
          3. Two-bar trailing stop breach
          4. Profit-lock tightening
        """
        ...
        # ── CHANGE 3: MR-specific exit path ───────────────────────────────────
        if pos.get('is_mr', False):
            unrealised_pct = side * (close - entry_px) / entry_px if entry_px > 0 else 0.0

            # (a) Hard time-stop — exit after mr_max_hold_bars bars regardless
            mr_max = cfg.get('mr_max_hold_bars', 15)
            if bars_held >= mr_max:
                return True

            # (b) Profit target — take the bounce when it arrives
            mr_target = cfg.get('mr_profit_target', 0.04)
            if unrealised_pct >= mr_target:
                return True

            # (c) Mean-reversion completion — close has crossed back through SMA.
            # For a long: entry was below SMA (oversold), exit when close >= SMA.
            # For a short: entry was above SMA (overbought), exit when close <= SMA.
            try:
                sma_val = float(self.evaluators[pos['config']].sma[d][0])
                if side > 0 and close >= sma_val:
                    return True
                if side < 0 and close <= sma_val:
                    return True
            except Exception:
            ...
            # (d) Tight trailing stop with two-bar confirmation
            mr_atr_k = cfg.get('mr_atr_k', 0.8)
            breached = (side > 0 and close < trail) or (side < 0 and close > trail)
            if breached:
                pos['breach_count'] += 1
                if pos['breach_count'] >= 2:
                    return True
            else:
                pos['breach_count'] = 0
                if side > 0:
                    pos['trail'] = max(trail, close - mr_atr_k * atr_val)
                else:
                    pos['trail'] = min(trail, close + mr_atr_k * atr_val)

            return False
        # ── End MR exit path ──────────────────────────────────────────────────

        # Original TF exit path (unchanged)
        severe_k = cfg['severe_breach_mult'] * atr_val

        # 1. Hard max holding period
    ...
    def next(self):
        ...
        # TF laggards — narrow pool used only for TF cross-sectional ranking (unchanged)
        bottom_cut = max(1, int(CONFIGS['LB60']['top_pct'] * n))
        laggards = set(ranked[:bottom_cut])

        # CHANGE 2: MR candidates — wider pool (mr_top_pct=0.30) so that 3 of 10 assets
        # are eligible for MR long on any bar.  Separate from laggards so widening MR
        # eligibility does not accidentally loosen TF cross-sectional filtering.
        mr_bottom_cut = max(1, int(CONFIGS['LB60']['mr_top_pct'] * n))
        mr_candidates = set(ranked[:mr_bottom_cut])
    ...
    def notify_order(self, order):
        ...
        is_mr = 'mean_reversion' in entry_type
        # CHANGE 3: MR positions use a tighter ATR multiplier at entry so the initial
        # stop is closer to price.  TF positions keep the wide atr_k they need to
        # survive normal trend noise without being stopped out prematurely.
        trail_k = cfg.get('mr_atr_k', cfg['atr_k']) if is_mr else cfg['atr_k']
        if size_sign > 0:
            # Long: trail starts below entry price
            trail = price - trail_k * atr_val
        else:
        ...
        self._pos[sym] = {
            ...
            'breach_count': 0,
            'config': config,
            # CHANGE 3: flag so check_exit can apply MR-specific rules
            'is_mr': is_mr,
        }
        if d in self.state:
