# strategies/xs_meanrev_balanced.py
# PD-Ratio = 2.313
# True_PD = 0.732
# Activity = 68.2%
...
class XsMeanRevBalanced(bt.Strategy):
    """
    Balanced cross-sectional mean reversion trading strategy with relaxed selection criteria.
    
    Entry Logic:
    - Identify laggards: bottom 15% of symbols by N-bar return
    - Require reversal: close > previous close AND close > open
    - No additional volume or true range gates (simplified entry)
    ...
    params = dict(
        ...
        entry_units_cap=400,               # Max initial units on entry (before ATR sizing)
        risk_notional_per_leg=25000.0,     # Risk target, used to derive units from ATR

        # Mean Reversion Selection
        meanrev_window=7,                  # N-bar lookback for selecting laggards
        bottom_pct=0.15,                   # Bottom 15%: relaxed selection vs 10%
    ...
    def ready(self):
        """Check if there are enough historical bars for all indicators."""
        need = max(7, int(self.p.meanrev_window), int(self.p.atr_len))
    ...
    def next(self):
        ...
        # Entry Logic: Balanced cross-sectional mean reversion (relaxed selection)
