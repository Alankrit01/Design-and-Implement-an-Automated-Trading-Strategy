'''
python main.py --strategy FixedShort --data-dir DATA/PART1 --output-dir output/BAYESTEST
'''

'''
{
  "final_value": 1011150.543512,
  "bankrupt": false,
  "bankrupt_date": null,
  "open_pnl_pd_ratio": 0.2866537109650868,
  "true_pd_ratio": 0.2691900343440161,
  "activity_pct": 94.3,
...
class BayesianTradingStrategy(bt.Strategy):
    params = dict(
        ...
        signal_learning_enabled=False,    # Learn which entry signals are reliable
    ...
    def eval_mr_signals(self,d):
        ...
        Signals checked:
        Signal 1: Stock down 2% in last 5 bars = oversold candidate
        Signal 2: Stock hasn't bounced yet (long_return filter)
        Signal 3: Volume confirmed
        Signal 4: Stock near 20-bar low = potential support (Bounce potential)
        
        - Need atleast 2 of 4 signals if Bayesian learning enabled
        - Posterior accuracy must be >42% (learning threshold)
        """
        ...
        # Count valid signals
        signal_count = sum(1 for v in mr_signals.values() if v)
        
        # Bayesian Learning
        if self.p.signal_learning_enabled:
            signal_accuracy = self.mr_signal_strength[d].get_accuracy()    # Posterior Estimate
            confidence_ok = signal_accuracy >= self.p.min_confidence_mr    # Threshold of 42% posterior accuracy
            signals_ok = signal_count >= 2  # need 2/4 signals
        else:
            confidence_ok = True
            signals_ok = all(mr_signals.values())   # Learning disabled, need all signals
        
        return confidence_ok and signals_ok, mr_signals
    ...
    def eval_tf_signals(self,d):
        ...
        brk_ok = close >= float(self.highest[d][-1]) if self.highest[d] is not None else False
        ...
        signal_count = sum(1 for v in trend_signals.values() if v)
        
        if self.p.signal_learning_enabled:
            signal_accuracy = self.tf_signal_strength[d].get_accuracy()     # Posterior Estimate
            confidence_ok = signal_accuracy >= self.p.min_confidence_trend  # Threshold of 42% posterior accuracy
            signals_ok = signal_count >= 3
        else:
            confidence_ok = True
            signals_ok = all(trend_signals.values())    # Learning disabled, need all signals
        
        return confidence_ok and signals_ok, trend_signals
    ...
    def next(self):
        ...
        for d in self.datas:
            ...
            if regime['sideways'] >= self.p.sideways_threshold:
                ...
                # Short: strong recent winners in sideways/bearish regime
            ...
            # TREND FOLLOWING (decide direction first)
